...
class BundleMetadata
{
    ...
    protected function buildInformation()
    {
        ...
        $this->extendedDirectory = sprintf('%s%s%s',
            str_replace(':vendor', $information[0], $this->configuration['application_dir']),
            DIRECTORY_SEPARATOR,
            $information[1]
        );
        $this->extendedNamespace = sprintf('%s\\%s',
            str_replace(':vendor', $information[0], $this->configuration['namespace']),
            $information[1]
        );
        $this->valid = true;
